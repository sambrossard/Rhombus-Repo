...
class GetVideoWall:
    ...
    def filter_wall(self, wall_json): 
        uuid_dump = []
        if wall_json and 'videoWalls' in wall_json: 
            video_walls = wall_json['videoWalls']
            for wall in video_walls:
                display_name = wall.get('displayName', '').lower()
                if 'test wall delete' in display_name:
                    uuid_dump.append(wall['uuid'])
                    pprint.pprint(uuid_dump)
        else:
            print('No video walls with this display name')
        return uuid_dump 
...
def run_cleanse():
    ...
    # delete_walls = get_walls.delete_video_walls()
